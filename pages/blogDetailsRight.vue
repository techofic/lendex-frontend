
<template>
    <div>
        <!--************Blog-Details-Right-Header****************-->

        <div class="breadcrumb-section overflow-hidden pos-relative">
            <div class="breadcrumb-shape-top-left">
            </div>
            <div class="breadcrumb-shape-bottom-right">
            </div>
            <div class="breadcrumb-box">
                <div class="container">
                    <div class="row">
                        <div class="col-12">
                            <div class="breadcrumb-content">
                                <h2 class="title">Blog Details Right Sidebar</h2>
                                <ul class="breadcrumb-link">
                                    <li>
                                        <a href="#">Home</a>
                                    </li>
                                    <li class="active">
                                        Blog Details Right Sidebar
                                    </li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <!--************Blog-Details-Right-Body****************-->

        <div class="blog-list-box pb-135 pt-135">
            <div class="container">
                <div class="row flex-lg-row flex-column-reverse">
                    <div class="col-xl-7 col-lg-7 offset-xl-1">
                        <div class="section-header default-content-style">
                            <h4 class="content-meta">
                                <div class="section-tag">UI/UX Design</div>
                                <div class="post-meta-2">
                                    <span class="icon-space-right">
                                        <i class="las la-user"></i>
                                        by Mallie Short
                                    </span>
                                    <span class="icon-space-right">
                                        <i class="las la-calendar"></i>
                                        09 Feb, 2021
                                    </span>
                                </div>
                            </h4>
                            <h4 class="title">
                                UI design can make easy your complex product.
                            </h4>
                            <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has
                                been the
                                industry's standard dummy text ever since the 1500s, when an unknown print took a galley of
                                type and
                                scrambled it to make a type specimen book. It has survived not only five centuries, but also
                                the
                                leap into electronic.
                            </p>
                            <h4 class="quote">
                                Lorem Ipsum is simply dummy text of the printing and typesetting indust has been the
                                industry's
                                standard dummy text ever since the 1500s, when an took a galley of type and scrambled
                                electronic.
                            </h4>
                            <p>
                                Lorem Ipsum is simply dummy text of the printing and typesetting indust has been the
                                industry's
                                standard dummy text ever since the 1500s, when an took a galley of type and scrambled
                                electronic.
                            </p>
                            <div class="content-inner-image image-double">
                                <div class="image">
                                    <img src="/assets/img/blog-details-content-inner-img-1.png" alt="">
                                </div>
                                <div class="image">
                                    <img src="/assets/img/blog-details-content-inner-img-2.png" alt="">
                                </div>
                            </div>
                            <h2 class="title">
                                It is essential to know whether the user getting.
                            </h2>
                            <p>
                                Lorem Ipsum is simply dummy text of the printing and typesetting indust has been the
                                industry's
                                standard dummy text ever since the 1500s, when an took a galley of type and scrambled
                                electronic.
                            </p>
                            <p>
                                Lorem Ipsum is simply dummy text of the printing and typesetting indust has been the
                                industry's
                                standard dummy text ever since the 1500s, when an took a galley of type and scrambled
                                electronic.
                            </p>
                            <h4 class="quote">
                                Lorem Ipsum is simply dummy text of the printing and typesetting indust has been the
                                industry's
                                standard dummy text ever since the 1500s, when an took a galley of type and scrambled
                                electronic.
                            </h4>
                            <ul class="content-list-item content-list-with-text">
                                <li>
                                    <div class="left">
                                        <h4 class="list-title">Sketching</h4>
                                    </div>
                                    <div class="right">
                                        <p>
                                            Lorem Ipsum is simply dummy text of the printing and has been the industry's
                                            standard
                                            dummy text ever took a galley of type make.
                                        </p>
                                    </div>
                                </li>
                                <li>
                                    <div class="left">
                                        <h4 class="list-title">UI Design</h4>
                                    </div>
                                    <div class="right">
                                        <p>
                                            Lorem Ipsum is simply dummy text of the printing and has been the industry's
                                            standard
                                            dummy text ever took a galley of type make.
                                        </p>
                                    </div>
                                </li>
                                <li>
                                    <div class="left">
                                        <h4 class="list-title">Development</h4>
                                    </div>
                                    <div class="right">
                                        <p>
                                            Lorem Ipsum is simply dummy text of the printing and has been the industry's
                                            standard
                                            dummy text ever took a galley of type make.
                                        </p>
                                    </div>
                                </li>
                            </ul>
                            <p>
                                Lorem Ipsum is simply dummy text of the printing and typesetting indust has been the
                                industry's
                                standard dummy text ever since the 1500s, when an took a galley of type and scrambled
                                electronic.
                            </p>
                            <p>
                                Lorem Ipsum is simply dummy text of the printing and typesetting indust has been the
                                industry's
                                standard dummy text ever since the 1500s, when an took a galley of type and scrambled
                                electronic.
                            </p>
                        </div>

                        <div class="tag-box">
                            <div class="left">
                                <div class="tag-list">
                                    <div class="title">Tags:</div>
                                    <ul class="list-item">
                                        <li><a href="#">portfolio,</a></li>
                                        <li><a href="#">charity,</a></li>
                                        <li><a href="#">personal</a></li>
                                    </ul>
                                </div>
                            </div>
                            <div class="right">
                                <div class="social-link">
                                    <a href="#"><i class="lab la-facebook-f"></i></a>
                                    <a href="#"><i class="lab la-dribbble"></i></a>
                                    <a href="#"><i class="lab la-linkedin-in"></i></a>
                                </div>
                            </div>
                        </div>

                        <div class="comment-box">
                            <h3 class="title">Comment:</h3>
                            <div class="comment-list-item">
                                <li>
                                    <div class="comment-single-item">
                                        <div class="image">
                                            <img src="/assets/img/user-1.png" alt="">
                                        </div>
                                        <div class="content">
                                            <div class="top">
                                                <div class="author-meta">
                                                    <h4 class="name">Felix Myers</h4>
                                                    <span class="designation">Web Developer</span>
                                                </div>
                                                <button class="replay-btn icon-space-right">
                                                    <i class="las la-reply"></i>Reply
                                                </button>
                                            </div>
                                            <div class="bottom">
                                                <div class="text">
                                                    <p>
                                                        Lorem Ipsum is simply dummy text of the printing and typesetting and
                                                        has
                                                        been the industry's standard dummy text ever since the 1500s, whe
                                                        took a
                                                        galley of type and scrambled.
                                                    </p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </li>
                                <li>
                                    <div class="comment-single-item">
                                        <div class="image">
                                            <img src="/assets/img/user-2.png" alt="">
                                        </div>
                                        <div class="content">
                                            <div class="top">
                                                <div class="author-meta">
                                                    <h4 class="name">Francisco Bond
                                                    </h4>
                                                    <span class="designation">Web Developer</span>
                                                </div>
                                                <button class="replay-btn icon-space-right">
                                                    <i class="las la-reply"></i>Reply
                                                </button>
                                            </div>
                                            <div class="bottom">
                                                <div class="text">
                                                    <p>
                                                        Lorem Ipsum is simply dummy text of the printing and typesetting and
                                                        has
                                                        been the industry's standard dummy text ever since the 1500s, whe
                                                        took a
                                                        galley of type and scrambled.
                                                    </p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </li>
                                <li>
                                    <div class="comment-single-item">
                                        <div class="image">
                                            <img src="/assets/img/user-3.png" alt="">
                                        </div>
                                        <div class="content">
                                            <div class="top">
                                                <div class="author-meta">
                                                    <h4 class="name">Rochell Duckett
                                                    </h4>
                                                    <span class="designation">Web Developer</span>
                                                </div>
                                                <button class="replay-btn icon-space-right">
                                                    <i class="las la-reply"></i>Reply
                                                </button>
                                            </div>
                                            <div class="bottom">
                                                <div class="text">
                                                    <p>
                                                        Lorem Ipsum is simply dummy text of the printing and typesetting and
                                                        has
                                                        been the industry's standard dummy text ever since the 1500s, whe
                                                        took a
                                                        galley of type and scrambled.
                                                    </p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </li>
                            </div>
                        </div>

                        <div class="comment-form-box">
                            <h3 class="title">Leave a comment:</h3>
                            <form class="default-form" action="#">
                                <div class="row mb-n6">
                                    <div class="col-xl-6 mb-6">
                                        <div class="default-form-group">
                                            <input type="text" placeholder="Name" required>
                                        </div>
                                    </div>
                                    <div class="col-xl-6 mb-6">
                                        <div class="default-form-group">
                                            <input type="email" placeholder="Email" required>
                                        </div>
                                    </div>
                                    <div class="col-xl-12 mb-6">
                                        <div class="default-form-group">
                                            <textarea placeholder="Comment" required></textarea>
                                        </div>
                                    </div>
                                    <div class="col-xl-12 mb-6">
                                        <div class="default-form-group">
                                            <button type="submit" class="main-button comment-btn">Submit Comment</button>
                                        </div>
                                    </div>
                                </div>
                            </form>
                        </div>
                    </div>
                    <div class="col-xl-4 col-lg-5">
                        <div class="sidebar-widget-area">
                            <div class="sidebar-widget-single-area search-widgets">
                                <form class="search-widgets-box" action="#">
                                    <input type="search" placeholder="Search here">
                                    <button>
                                        <i class="las la-search"></i>
                                    </button>
                                </form>
                            </div>
                            <div class="sidebar-widget-single-area category-widgets">
                                <h3 class="title">Category</h3>
                                <ul class="widget-nav-list category-item-list">
                                    <li>
                                        <a href="#">
                                            <span class="text">UI/UX Design</span>
                                            <span class="icon"><i class="las la-angle-double-right"></i></span>
                                        </a>
                                    </li>
                                    <li>
                                        <a href="#">
                                            <span class="text">Web Development</span>
                                            <span class="icon"><i class="las la-angle-double-right"></i></span>
                                        </a>
                                    </li>
                                    <li>
                                        <a href="#">
                                            <span class="text">Brand Identity</span>
                                            <span class="icon"><i class="las la-angle-double-right"></i></span>
                                        </a>
                                    </li>
                                    <li>
                                        <a href="#">
                                            <span class="text">Digital Marketing</span>
                                            <span class="icon"><i class="las la-angle-double-right"></i></span>
                                        </a>
                                    </li>
                                </ul>
                            </div>
                            <div class="sidebar-widget-single-area resume-widgets">
                                <h3 class="title">Resume</h3>
                                <ul class="widget-nav-list category-item-list">
                                    <li>
                                        <a href="#">
                                            <span class="text">Get Resume.pdf</span>
                                            <span class="icon"><i class="las la-file-pdf"></i></span>
                                        </a>
                                    </li>
                                    <li>
                                        <a href="#">
                                            <span class="text">Get Resume.doc</span>
                                            <span class="icon"><i class="las la-file-word"></i></span>
                                        </a>
                                    </li>
                                    <li>
                                        <a href="#">
                                            <span class="text">Get Resume.ppt</span>
                                            <span class="icon"><i class="las la-file-powerpoint"></i></span>
                                        </a>
                                    </li>
                                </ul>
                            </div>
                            <div class="sidebar-widget-single-area recentpost-widgets">
                                <h3 class="title">Recent Post</h3>
                                <ul class="recent-blog-item-list">
                                    <li>
                                        <a href="#" class="image">
                                            <img src="/assets/img/recent-blog-img-1.png" alt="">
                                        </a>
                                        <div class="content">
                                            <h4 class="title">
                                                <a href="#">
                                                    Mistakes You Can Easily Avoid.
                                                </a>
                                            </h4>
                                            <div class="post-meta">
                                                <i class="las la-calendar"></i>
                                                09 Apr, 2021
                                            </div>
                                        </div>
                                    </li>
                                    <li>
                                        <a href="#" class="image">
                                            <img src="/assets/img/recent-blog-img-2.png" alt="">
                                        </a>
                                        <div class="content">
                                            <h4 class="title">
                                                <a href="#">
                                                    Mistakes You Can Easily Avoid.
                                                </a>
                                            </h4>
                                            <div class="post-meta">
                                                <i class="las la-calendar"></i>
                                                09 Apr, 2021
                                            </div>
                                        </div>
                                    </li>
                                    <li>
                                        <a href="#" class="image">
                                            <img src="/assets/img/recent-blog-img-3.png" alt="">
                                        </a>
                                        <div class="content">
                                            <h4 class="title">
                                                <a href="#">
                                                    Mistakes You Can Easily Avoid.
                                                </a>
                                            </h4>
                                            <div class="post-meta">
                                                <i class="las la-calendar"></i>
                                                09 Apr, 2021
                                            </div>
                                        </div>
                                    </li>
                                    <li>
                                        <a href="#" class="image">
                                            <img src="/assets/img/recent-blog-img-4.png" alt="">
                                        </a>
                                        <div class="content">
                                            <h4 class="title">
                                                <a href="#">
                                                    Mistakes You Can Easily Avoid.
                                                </a>
                                            </h4>
                                            <div class="post-meta">
                                                <i class="las la-calendar"></i>
                                            09 Apr, 2021
                                        </div>
                                    </div>
                                </li>
                            </ul>
                        </div>
                        <div class="sidebar-widget-single-area">
                            <h3 class="title">Follow Me</h3>
                            <div class="social-link">
                                <a href="#"><i class="lab la-facebook-f"></i></a>
                                <a href="#"><i class="lab la-dribbble"></i></a>
                                <a href="#"><i class="lab la-linkedin-in"></i></a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div></template>

</html>